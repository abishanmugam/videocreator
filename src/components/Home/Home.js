...
import HeroImage from "../elements/Herolmage/HeroImage";
...
const Home = () => {
  ...
  const { API_URL, API_KEY, IMAGE_BASE_URL, BACKDROP_SIZE, POSTER_SIZE } =
    apiConfig;
  ...
  const loadMoreItems = () => {
    ...
    if (!searchTerm) {
      ...
      endpoint = `${API_URL}search/movie?api_key=${API_KEY}&query=${searchTerm}&page=${
        currentPage + 1
      }`;
    }
    fetchMovies(endpoint);
